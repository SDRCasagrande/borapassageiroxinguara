...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
function MotoristaHero({ data }: { data?: any }) {
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
        ...
        <div className="space-y-8">
          ...
          >
            ...
            {data?.badge || 'Vagas Abertas na Região do Xingu'}
          </motion.div>
          ...
          >
            {data?.subtitle ? (
              <span dangerouslySetInnerHTML={{ __html: data.subtitle.replace('10%', '<span class="text-emerald-400 font-bold">' + (data.taxRate || '10%') + '</span>') }} />
            ) : (
          ...
          >
            ...
            >
              ...
              {data?.secondaryCtaText || 'Simular Ganhos'}
            </a>
          </motion.div>
...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
function FintechBalanceCard({ data }: { data?: any }) {
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          <h2 className="text-3xl sm:text-5xl font-black text-white tracking-tight">
            {data?.title || 'Controle Total dos Seus'}{' '}
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-teal-400">
              {data?.titleGradient || 'Ganhos'}
            </span>
          </h2>
          <p className="mt-4 text-lg text-emerald-50/60 max-w-2xl mx-auto">
            {data?.subtitle || 'Acompanhe em tempo real cada corrida, taxa e saldo. Tudo transparente, direto no app.'}
          </p>
        </motion.div>
        ...
        >
          <div className="relative bg-gradient-to-br from-[#1a2332] to-[#0c1322] border border-white/10 p-8 sm:p-10 rounded-[2rem] shadow-2xl overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-10 relative z-10">
              <div>
                <p className="text-emerald-400 font-bold text-xs tracking-[0.2em] uppercase mb-2">
                  {data?.balanceLabel || 'Seu Saldo'}
                </p>
                <h3 className="text-5xl sm:text-6xl font-black text-white tracking-tighter">
                  {data?.balanceValue?.split(',')[0] || 'R$ 1.845'}
                  <span className="text-2xl text-white/40">,{data?.balanceValue?.split(',')[1] || '50'}</span>
                </h3>
              </div>
              <motion.div
            ...
            <div className="space-y-3 relative z-10">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-semibold text-sm">{data?.rows?.[0]?.label || 'Corridas Hoje'}</p>
                    <p className="text-xs text-white/40">{data?.rows?.[0]?.subLabel || '24 finalizadas'}</p>
                  </div>
                </div>
                <span className="text-emerald-400 font-bold text-lg">{data?.rows?.[0]?.value || '+ R$ 340,00'}</span>
              </motion.div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-semibold text-sm">{data?.rows?.[1]?.label || 'Taxa Bora'}</p>
                    <p className="text-xs text-white/40">{data?.rows?.[1]?.subLabel || 'Taxas a partir de 10% do faturamento'}</p>
                  </div>
                </div>
                <span className="text-white/40 font-bold text-lg">{data?.rows?.[1]?.value || '- R$ 34,00'}</span>
              </motion.div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-emerald-100 font-semibold text-sm">{data?.rows?.[2]?.label || 'Líquido no Bolso'}</p>
                    <p className="text-xs text-emerald-400/60">{data?.rows?.[2]?.subLabel || 'Seu dinheiro de verdade'}</p>
                  </div>
                </div>
                <span className="text-emerald-400 font-black text-xl">{data?.rows?.[2]?.value || 'R$ 306,00'}</span>
              </motion.div>
            </div>
            ...
            >
              <div className="text-sm text-white/40 font-medium">{data?.bottomLabel || 'Desempenho Semanal'}</div>
              <div className="inline-flex items-center gap-1.5 text-emerald-400 font-bold text-sm bg-emerald-500/10 px-3 py-1.5 rounded-full">
                ...
                {data?.bottomValue || '+24% vs semana anterior'}
              </div>
            </motion.div>
          </div>
        </motion.div>
...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
function EarningsCalculator({ data }: { data?: any }) {
  const [rides, setRides] = useState(data?.defaultRides || 15);
  const averageTicket = data?.averageTicket || 15;
  const totalGross = rides * averageTicket;
  const boraFee = totalGross * ((data?.feePercent || 10) / 100);
  const netDay = totalGross - boraFee;
  const netMonth = netDay * (data?.workingDays || 25);
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold uppercase tracking-widest mb-6">
            ...
            {data?.badge || 'Simulador de Ganhos'}
          </div>
          <h2 className="text-3xl sm:text-5xl font-black text-white tracking-tight">
            {data?.title || 'Simulador'}{' '}
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-teal-400">
              {data?.titleGradient || 'Transparente'}
            </span>
          </h2>
          <p className="mt-4 text-lg text-emerald-50/60 max-w-2xl mx-auto">
            {data?.subtitle || 'Sem taxas ocultas, sem letras miúdas. Simule os ganhos com base na nossa taxa a partir de 10%.'}
          </p>
        </motion.div>
...
/* ═══════════════════════════════════════════════════════════
   5. REQUISITOS SECTION
   ═══════════════════════════════════════════════════════════ */
function RequisitosSection({ data }: { data?: any }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: '-60px' });

  const requisitos = data?.items || [
    {
      title: 'CNH Definitiva',
      description: 'Com observação Exerce Atividade Remunerada (EAR) obrigatória.',
    },
    {
      title: 'Idade 21+',
      description: '21 anos completos no momento do cadastro para segurança de todos.',
    },
    {
      title: 'Veículo 4 Portas',
      description: 'Carro com 4 portas, ar-condicionado e até 10 anos de fabricação.',
    },
    {
      title: 'CRLV Digital',
      description: 'Licenciamento em dia. Documento digital atualizado obrigatório.',
    },
  ];

  // We assign icons dynamically to the items
  const iconSet = [
    { icon: <FileText className="w-6 h-6" />, gradient: 'from-emerald-500/20 to-emerald-600/5', iconColor: 'text-emerald-400' },
    { icon: <Shield className="w-6 h-6" />, gradient: 'from-teal-500/20 to-teal-600/5', iconColor: 'text-teal-400' },
    { icon: <Car className="w-6 h-6" />, gradient: 'from-cyan-500/20 to-cyan-600/5', iconColor: 'text-cyan-400' },
    { icon: <CheckCircle2 className="w-6 h-6" />, gradient: 'from-emerald-500/20 to-emerald-600/5', iconColor: 'text-emerald-400' },
  ];
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold uppercase tracking-widest mb-6">
            ...
            {data?.badge || 'Pré-Requisitos'}
          </div>
          <h2 className="text-3xl sm:text-5xl font-black text-white tracking-tight">
            {data?.title || 'O Que Você'}{' '}
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-teal-400">
              {data?.titleGradient || 'Precisa'}
            </span>
          </h2>
          <p className="mt-4 text-emerald-50/60 font-medium max-w-lg mx-auto">
            {data?.subtitle || 'Pouca burocracia, muita segurança. Padrão exigido para manter nossa frota premium.'}
          </p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {requisitos.map((req: any, i: number) => {
            const iconData = iconSet[i % iconSet.length];
            return (
              <motion.div
                key={i}
                custom={i}
              ...
              >
                {/* Hover glow */}
                <div className={`absolute inset-0 bg-gradient-to-b ${iconData.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none`} />
                ...
                <div className="relative z-10">
                  <div className={`w-14 h-14 bg-emerald-500/10 group-hover:bg-emerald-500/20 rounded-2xl flex items-center justify-center mb-6 transition-colors duration-300 ${iconData.iconColor}`}>
                    {iconData.icon}
                  </div>
...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
function TestimonialCTA({ data }: { data?: any }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: '-60px' });
          ...
          >
            <div className="relative bg-gradient-to-br from-[#1a2332] to-[#0c1322] border border-white/10 p-8 sm:p-10 rounded-[2rem] shadow-2xl overflow-hidden">
              ...
              <p className="text-white/80 text-lg sm:text-xl leading-relaxed font-medium mb-8">
                {data?.quote ? (
                  <span dangerouslySetInnerHTML={{ __html: data.quote.replace('tenho a parceria mais justa', '<span class="text-emerald-400 font-bold">tenho a parceria mais justa</span>') }} />
                ) : (
                  <>Eu rodava em outro app e pagava taxas abusivas que consumiam meu faturamento. No Bora,{' '}
                  <span className="text-emerald-400 font-bold">tenho a parceria mais justa</span> do mercado. Minha renda
                  aumentou sem precisar trabalhar mais horas. É o app que realmente valoriza o motorista.</>
                )}
              </p>
              ...
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full overflow-hidden border-2 border-emerald-500/30 flex-shrink-0">
                  <img
                    src={data?.quoteImage || "/assets/driver-profile.png"}
                    alt={data?.quoteName || "Motorista parceiro"}
                    className="w-full h-full object-cover"
                  />
                </div>
                <div>
                  <p className="text-white font-bold">{data?.quoteName || 'Carlos Augusto'}</p>
                  <p className="text-emerald-400/70 text-sm font-medium">
                    {data?.quoteSubtitle || 'Motorista Parceiro • Xinguara-PA'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1 mt-6 pt-6 border-t border-white/5">
                ...
                <span className="text-white/40 text-sm ml-2 font-medium">{data?.quoteRating || '5.0 Avaliação no App'}</span>
              </div>
            </div>
          ...
          >
            <div>
              <h2 className="text-3xl sm:text-5xl font-black text-white tracking-tight leading-tight">
                {data?.heading || 'Pronto para'}{' '}
                <span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-teal-400">
                  {data?.headingGradient || 'ganhar mais?'}
                </span>
              </h2>
              <p className="mt-5 text-lg text-emerald-50/60 leading-relaxed">
                {data?.subtitle || 'Faça seu pré-cadastro agora e comece a rodar com a plataforma que mais valoriza o motorista no Xingu. Processo 100% digital e rápido.'}
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              ...
              >
                {data?.ctaButton || 'Preencher Pré-Cadastro'}
                <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Link>
              <a
              ...
              >
                ...
                {data?.whatsappButton || 'Falar no WhatsApp'}
...
/* ═══════════════════════════════════════════════════════════
   PAGE EXPORT
   ═══════════════════════════════════════════════════════════ */
export default function MotoristaClient({ content = {} }: { content?: any }) {
  return (
    <div className="bg-[#030712] min-h-screen selection:bg-emerald-500/30 selection:text-emerald-50 antialiased">
      <PremiumHeaderMotorista data={content.config} />

      <main>
        {/* 2. Hero */}
        <MotoristaHero data={content.hero} />
        {/* 3. Fintech Balance Card */}
        <FintechBalanceCard data={content.fintech} />
        {/* 4. Earnings Calculator */}
        <EarningsCalculator data={content.calculator} />
        {/* 5. Requisitos */}
        <RequisitosSection data={content.requisitos} />
        {/* 6. Depoimentos e FAQ */}
        <Testimonials data={content.testimonials} />
        <FAQ data={content.faq} />
        {/* 7. CTA Final */}
        <TestimonialCTA data={content.cta} />
      </main>
      ...
      {/* 8. Floating WhatsApp */}
      <FloatingWhatsApp data={content.config} />
    </div>
  );
}
